...
/// </summary>
public sealed class ImportacionDatosService(IDbContextFactory<ArgentaDbContext> fabricaDb) : IImportacionDatosService
{
    ...
    public DatosArgentaJson ParsearJson(string contenidoJson)
    {
        DatosArgentaJson? datos;
        try
        {
            datos = JsonSerializer.Deserialize<DatosArgentaJson>(contenidoJson, OpcionesJson);
        }
        ...
        if (datos.Version != 1)
        {
            throw new FormatException(
                $"Versión de archivo no soportada ({datos.Version}). Esta versión de Argenta solo admite archivos de versión 1.");
        }

        return datos;
    }

    public Task<ResumenImportacion> PrevisualizarAsync(DatosArgentaJson datos) => ProcesarAsync(datos, aplicarCambios: false);

    public Task<ResumenImportacion> ImportarAsync(DatosArgentaJson datos) => ProcesarAsync(datos, aplicarCambios: true);
    ...
    public string GenerarPlantillaVacia()
    {
        ...
        var plantilla = new DatosArgentaJson
        {
            Version = 1,
            Clientes =
            [
    ...
    /// </summary>
    private async Task<ResumenImportacion> ProcesarAsync(DatosArgentaJson datos, bool aplicarCambios)
    {
