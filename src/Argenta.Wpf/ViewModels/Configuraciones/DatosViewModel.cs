...
namespace Argenta.Wpf.ViewModels.Configuraciones;
...
public partial class DatosViewModel(IImportacionDatosService servicio) : ObservableObject
{
    ...
    private async Task ImportarDatosAsync()
    {
        ...
        {
            MessageBox.Show($"No se pudo leer el archivo: {ex.Message}", "Argenta", MessageBoxButton.OK, MessageBoxImage.Error);
            return;
        }
        ...
        {
            MessageBox.Show($"No se pudo revisar el archivo: {ex.Message}", "Argenta", MessageBoxButton.OK, MessageBoxImage.Error);
            return;
        }
        ...
        if (resumen.TotalNuevos == 0 && resumen.TotalActualizados == 0)
        {
            MessageBox.Show(
                ...
                "Argenta", MessageBoxButton.OK, MessageBoxImage.Information);
            return;
        }
    ...
    private void DescargarPlantilla()
    {
        ...
        {
            Filter = "Archivo de datos Argenta (*.json)|*.json",
            FileName = "Argenta - plantilla de datos.json",
        };
        if (dialogo.ShowDialog() != true) return;
        ...
        {
            MessageBox.Show($"No se pudo guardar la plantilla: {ex.Message}", "Argenta", MessageBoxButton.OK, MessageBoxImage.Error);
        }
    }
