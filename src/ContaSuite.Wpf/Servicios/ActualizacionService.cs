...
public sealed class ActualizacionService(IConfiguration configuracion) : IActualizacionService
{
    public async Task<string> BuscarYAplicarAsync()
    {
        ...
        try
        {
            ...
            {
                return "Esta copia no fue instalada con el instalador de ContaSuite; no se puede actualizar automáticamente.";
            }

            var actualizacionDisponible = await administrador.CheckForUpdatesAsync();
            if (actualizacionDisponible is null)
            {
                return "Ya tiene instalada la última versión de ContaSuite.";
            }
            ...
            return "Actualización descargada. ContaSuite se reiniciará para aplicarla.";
        }
        catch (Exception ex)
        {
